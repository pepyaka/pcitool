...
use crate::names::{CcKey, ClassCode, VdsKey, VendorDeviceSubsystem};
...
impl<'a> PciIds<'a> {
    ...
    #[cfg(feature = "precompiled_ids")]
    pub fn precompiled() -> Self {
        Self {
            lines: include_str!("/usr/share/hwdata/pci.ids").lines(),
            vendor: None,
            device: None,
            class: None,
            subclass: None,
...
impl Iterator for PciIds<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        for line in self.lines.by_ref() {
            ...
            {
                return Some(PciIdsEntry::Subsystem(
                    vendor.clone(),
                    device.clone(),
                    subsystem,
                ));
            ...
            {
                return Some(PciIdsEntry::ProgIf(
                    class.clone(),
                    subclass.clone(),
                    prog_if,
                ));
...
impl FromIterator<PciIdsEntry> for (VendorDeviceSubsystem, ClassCode) {
    fn from_iter<I: IntoIterator<Item = PciIdsEntry>>(iter: I) -> Self {
        let mut vds: HashMap<VdsKey, String> = HashMap::default();
        let mut cc: HashMap<CcKey, String> = HashMap::default();
        for entry in iter {
            match entry {
                PciIdsEntry::Vendor(vendor) => vds.insert(VdsKey::Vendor(vendor.id), vendor.name),
                ...
                PciIdsEntry::Class(class) => cc.insert(CcKey::Class(class.id), class.name),
                PciIdsEntry::Subclass(class, subclass) => {
                    cc.insert(CcKey::Subclass(class.id, subclass.id), subclass.name)
                }
                PciIdsEntry::ProgIf(class, subclass, prog_if) => cc.insert(
                    CcKey::ProgIf(class.id, subclass.id, prog_if.id),
                    prog_if.name,
                ),
            };
        }
...
mod tests {
    ...
    fn parse_vendor_valid() {
        let result = "aa55  Ncomputing X300 PCI-Engine"
            .parse::<Vendor>()
            .unwrap();
        assert_eq!(
            Vendor {
                id: 0xaa55,
                name: "Ncomputing X300 PCI-Engine".into()
            },
            result
        );
    }
    ...
    fn parse_vendor_invalid_hex() {
        let result = "ag55  Ncomputing X300 PCI-Engine"
            .parse::<Vendor>()
            .unwrap_err();
        assert_eq!(
            ParseLineError::Hex(u16::from_str_radix("z", 2).unwrap_err()),
            result
        );
    }
    ...
    fn parse_class_invalid_hex() {
        ...
        assert_eq!(
            ParseLineError::Hex(u8::from_str_radix("z", 2).unwrap_err()),
            result
        );
    }
    ...
    fn parse_subclass_valid() {
        ...
        assert_eq!(
            Subclass {
                id: 0x01,
                name: "Satellite TV controller".into()
            },
            result
        );
    }
    ...
    fn parse_subclass_invalid_hex() {
        let result = "	0Z  Satellite TV controller"
            .parse::<Subclass>()
            .unwrap_err();
        assert_eq!(
            ParseLineError::Hex(u16::from_str_radix("z", 2).unwrap_err()),
    ...
    fn parse_progif_invalid_match() {
        let result = "		001c 0004  2 Channel CAN Bus SJC1000"
            .parse::<ProgIf>()
            .unwrap_err();
        assert_eq!(ParseLineError::LineMatch, result);
    }
    ...
    #[test]
    fn pciids_iterator() {
        let data = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/data/pci.ids"));
    ...
    fn collect_pciids_cc_db() {
        ...
        assert_eq!(cc.lookup(0x01, 0x01, None).unwrap(), "IDE interface");
        assert_eq!(
            cc.lookup(0x01, 0x01, 0x05).unwrap(),
            "PCI native mode-only controller"
        );
        assert_eq!(cc.lookup(0xaa, 0xaa, 0xaa), None);
    }
}
