...
use crate::view::lspci::{Flag, VerboseView};

impl<'a> fmt::Display for VerboseView<&'a Dvsec<'a>> {
...
impl<'a> fmt::Display for VerboseView<&'a DvsecError> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let &VerboseView { data, .. } = self;
        match data {
