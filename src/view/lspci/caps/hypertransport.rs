...
impl<'a> DisplayMultiView<Verbose> for Hypertransport {}
...
fn fmt_pri<'a>(
    ...
    if rid >= 0x22 {
        write!(
            f,
            " DUL{}",
            cmd.drop_on_uninitialized_link.display(BoolView::PlusMinus)
        )?;
    }
    writeln!(f)?;
    write!(
    ...
    writeln!(
        f,
        "\t\tLink Frequency 0: {}",
        sopi.link_freq_0(false).display(())
    )?;
    writeln!(
        f,
    ...
    writeln!(
        f,
        "\t\tLink Frequency 1: {}",
        sopi.link_freq_1(false).display(())
    )?;
    writeln!(
        f,
...
impl<'a> DisplayMultiView<()> for LinkWidth {}
...
impl<'a> DisplayMultiView<()> for LinkFrequency {}
