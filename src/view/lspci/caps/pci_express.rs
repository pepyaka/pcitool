use crate::device;
use crate::device::capabilities::pci_express::CompletionTimeoutRanges;
use crate::device::capabilities::pci_express::CompletionTimeoutValue;
use crate::device::capabilities::pci_express::CompliancePresetOrDeEmphasis;
use crate::device::capabilities::pci_express::CrosslinkResolution;
use crate::device::capabilities::pci_express::DeEmphasis;
use crate::device::capabilities::pci_express::DownstreamComponentPresence;
use crate::device::capabilities::pci_express::EmergencyPowerReduction;
use crate::device::capabilities::pci_express::Link2;
use crate::device::capabilities::pci_express::Device2;
use crate::device::capabilities::pci_express::LnSystemCls;
use crate::device::capabilities::pci_express::Obff;
use crate::device::capabilities::pci_express::ObffEnable;
use crate::device::capabilities::pci_express::Root;
use crate::device::capabilities::pci_express::IndicatorControl;
use crate::device::capabilities::pci_express::Slot;
use crate::device::capabilities::pci_express::L1ExitLatency;
use crate::device::capabilities::pci_express::ActiveStatePowerManagement;
use crate::device::capabilities::pci_express::L0sExitLatency;
use crate::device::capabilities::pci_express::Link;
use crate::device::capabilities::pci_express::LinkSpeed;
use crate::device::capabilities::pci_express::LinkWidth;
use crate::device::capabilities::pci_express::MaxSize;
use crate::device::capabilities::pci_express::EndpointL1AcceptableLatency;
use crate::device::capabilities::pci_express::DeviceStatus;
use crate::device::capabilities::pci_express::DeviceControl;
use crate::device::capabilities::pci_express::Device;
use crate::device::capabilities::pci_express::DeviceCapabilities;
use crate::device::capabilities::pci_express::EndpointL0sAcceptableLatency;
use crate::device::capabilities::pci_express::ExtendedTagFieldSupported;
use crate::device::capabilities::pci_express::Slot2;
use crate::device::capabilities::pci_express::SupportedLinkSpeedsVector;
use crate::device::capabilities::pci_express::TphCompleter;
use crate::device::capabilities::pci_express::TransmitMargin;
use crate::view::DisplayMultiViewBasic;
use core::fmt;
use std::cmp::Ordering;

use crate::device::capabilities::pci_express::Capabilities;
use crate::device::capabilities::pci_express::DeviceType;
use crate::view::BoolView;
use crate::view::MultiView;
use crate::device::capabilities::PciExpress;


const LATENCY_L0S: [&'static str; 8] = [
    "<64ns", "<128ns", "<256ns", "<512ns", "<1us", "<2us", "<4us", "unlimited"
];
const LATENCY_L1: [&'static str; 8] = [
...
impl<'a> fmt::Display for MultiView<&'a PciExpress, PciExpressView<'a>> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let slot = slot_implemented.display(BoolView::PlusMinus);
        match device_type {
            DeviceType::Endpoint => write!(f, "Endpoint"),
            DeviceType::LegacyEndpoint => write!(f, "Legacy Endpoint"),
            DeviceType::RootPort => write!(f, "Root Port (Slot{})", slot),
            DeviceType::UpstreamPort => write!(f, "Upstream Port"),
            DeviceType::DownstreamPort => write!(f, "Downstream Port (Slot{})", slot),
            DeviceType::PcieToPciBridge => write!(f, "PCI-Express to PCI/PCI-X Bridge"),
            DeviceType::PciToPcieBridge => write!(f, "PCI/PCI-X to PCI-Express Bridge (Slot{})", slot),
        ...
        if let DeviceType::Endpoint | DeviceType::LegacyEndpoint |
            DeviceType::UpstreamPort | DeviceType::PciToPcieBridge = device_type
        {
           write!(f,
        ...
        if let DeviceType::Endpoint | DeviceType::UpstreamPort |
            DeviceType::PciToPcieBridge = device_type
        {
        ...
        if let DeviceType::PciToPcieBridge = device_type {
...
impl<'a> MultiView<&'a PciExpress, PciExpressView<'a>> {
    fn fmt_link(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(Link { capabilities: caps, control: ctrl, status: st }) = &self.data.link {
            ...
            if let DeviceType::RootPort | DeviceType::Endpoint |
                DeviceType::LegacyEndpoint | DeviceType::PciToPcieBridge = device_type
            {
            ...
            write!(f,
                ...
                match st.current_link_speed.cmp(&caps.max_link_speed) {
    ...
    fn fmt_dev2(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let device_type = &self.data.capabilities.device_type;
        if let Some(Device2 { capabilities: caps, control: ctrl, status: _st }) = self.data.device_2.as_ref()
            // lspci shows Device 2 all zeros anyway. TODO: Fix it
            .or(Some(&Device2 { capabilities: 0.into(), control: 0.into(), status: 0.into() }))
        {
            write!(f,
    ...
    fn fmt_link2(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(Link2 { capabilities: caps, control: ctrl, status: st }) = &self.data.link_2 {
            let device_type = &self.data.capabilities.device_type;
                ...
                (self.view.device.address.device != 0 || self.view.device.address.function != 0))
            {
                ...
                write!(f,
                    ...
                    ctrl.target_link_speed.display(()),
